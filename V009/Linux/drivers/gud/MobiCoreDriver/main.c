...
{
	MCDRV_ASSERT(NULL != used_l2table);
	MCDRV_ASSERT(NULL != used_l2table->set);
	MCDRV_ASSERT(NULL != used_l2table->set->kernel_virt);
...
)
{
	MCDRV_ASSERT(NULL != used_l2table);
	MCDRV_ASSERT(NULL != used_l2table->set);
	MCDRV_ASSERT(NULL != used_l2table->set->phys);
	...
	do {
		...
		list_for_each_entry(
			...
			for (i = 0; i < MC_DRV_KMOD_L2_TABLE_PER_PAGES; i++) {
				if (0 == (l2table_set->usage_bitmap & (1U << i))
				) {
					/* found a set,
...
)
{
	...
	if (0 == l2table_set->usage_bitmap) {
		MCDRV_ASSERT(NULL != l2table_set->page);
		ClearPageReserved(l2table_set->page);

		MCDRV_ASSERT(NULL != l2table_set->kernel_virt);
...
)
{
	...
	MCDRV_ASSERT(NULL != task); */
	MCDRV_ASSERT(NULL != wsm_buffer);
	MCDRV_ASSERT(0 != wsm_len);
	MCDRV_ASSERT(NULL != used_l2table);
	...
	do {
		...
		if (SZ_1M < (nr_of_pages*PAGE_SIZE)) {
...
)
{
	...
	MCDRV_DBG_VERBOSE("clear L2 table, physBase=%p, nr_of_pages=%d\n",
...
)
{
	...
	do {
		...
		if (0 != params.in.pid) {
			...
			pid_struct = find_get_pid(params.in.pid);
			if (NULL != pid_struct)
			{
...
)
{
	...
	do {
		...
		if (MC_DRV_KMOD_CONTG_BUFFER_MAX == i) {
...
)
{
	...
	do {
		...
		virt_kernel_addr_stack = (void *)__get_free_pages(
							GFP_USER | __GFP_COMP,
							order);
		if (NULL == virt_kernel_addr_stack) {
...
)
{
	...
	do {
		...
		if ((MC_DRV_KMOD_MMAP_MCI == request) && (mci_base != 0)) {
...
#if defined(DEBUG)
				/*virt_kernel_addr_stack =
							phys_to_virt(request);
				virt_kernel_addr_stack =
					ioremap(request,requested_size);*/
		...
		/* store MCI pointer */
		if ((MC_DRV_KMOD_MMAP_MCI == request) && (0 == mci_base)) {
...
)
{
	...
	do {
		...
		if ((is_userland_caller_privileged())
			&& (NULL == mc_drv_kmod_ctx.daemon_inst)) {
...
)
{
	...
	do {
		...
		for (i = 0; i < MC_DRV_KMOD_CONTG_BUFFER_MAX; i++) {
			...
			if (0 != contg_buffer->virt_user_addr) {
